...
namespace BlueCrown.Api.Services.Implementations
{
    public class PrescriptionService : IPrescriptionService
    {
        ...
        public PrescriptionService(
            ...
            IPrescriptionDispenseRepository dispenseRepository,
        ...
        public async Task<List<PrescriptionCheckoutOptionDto>> GetPatientCheckoutOptionsAsync(Guid userId)
        {
            var patientProfile = await GetPatientProfileAsync(userId);
            var prescriptions = await _prescriptionRepository.GetCheckoutOptionsByPatientIdAsync(patientProfile.Id);

            return prescriptions.Select(MapToCheckoutOptionDto).ToList();
        }
        ...
        public async Task<PrescriptionDto> CreateAsync(Guid userId, CreatePrescriptionDto dto)
        {
            ...
            var hasMedicalRecord =
                dto.MedicalRecordId.HasValue &&
                dto.MedicalRecordId.Value != Guid.Empty;

            var hasAppointment =
                dto.AppointmentId.HasValue &&
                dto.AppointmentId.Value != Guid.Empty;
            ...
            if (hasMedicalRecord)
            {
                medicalRecord = await _medicalRecordRepository.GetByIdAsync(
                    dto.MedicalRecordId!.Value);
                ...
                if (medicalRecord.DoctorId != doctorProfile.Id)
                    throw new InvalidOperationException(
                        "Bạn không có quyền kê đơn cho hồ sơ bệnh án này.");

                if (medicalRecord.Appointment == null)
                    throw new InvalidOperationException(
                        "Hồ sơ bệnh án không có lịch khám hợp lệ.");
                ...
                {
                    throw new InvalidOperationException(
                        "Hồ sơ bệnh án chỉ được sử dụng cho lịch khám trực tiếp.");
                }
                ...
                {
                    throw new InvalidOperationException(
                        "Chỉ hồ sơ từ lịch khám đã hoàn thành mới được kê đơn thuốc.");
                }

                if (hasAppointment &&
                    dto.AppointmentId!.Value != appointment.Id)
                {
                    throw new ArgumentException(
                        "Lịch khám không khớp với hồ sơ bệnh án.");
                }
            ...
            else
            {
                ...
                {
                    throw new InvalidOperationException(
                        "Khám trực tiếp phải có hồ sơ bệnh án trước khi kê đơn thuốc.");
                }
                ...
                {
                    throw new InvalidOperationException(
                        "Chỉ tư vấn trực tuyến đã hoàn thành mới được kê đơn thuốc.");
                }

                // BR-PRE-018: Đơn thuốc online phải có chẩn đoán.
                if (string.IsNullOrWhiteSpace(dto.Diagnosis))
                    throw new ArgumentException(
                        "Vui lòng nhập chẩn đoán cho đơn thuốc tư vấn trực tuyến.");
            ...
            var existingPrescription =
                await _prescriptionRepository.GetByAppointmentIdAsync(appointment.Id);

            if (existingPrescription != null)
                throw new InvalidOperationException(
                    "Lịch khám này đã có đơn thuốc.");
            ...
            prescription.PrescriptionItems = dto.Items
                .Select(item => new PrescriptionItem
                {
                    ...
                    Instructions = string.IsNullOrWhiteSpace(item.Instructions)
                        ? null
            ...
            if (created == null)
                throw new Exception(
                    "Không thể lấy đơn thuốc vừa tạo.");
        ...
        public async Task<PrescriptionDto?> GetPharmacistPrescriptionByIdAsync(Guid id)
        {
            ...
            // BR-PHA-PRE-004: Đơn online không thuộc luồng xử lý trực tiếp của Pharmacist.
            if (string.Equals(
        ...
        public async Task<PrescriptionDto?> UpdatePharmacistStatusAsync(
            Guid id,
            UpdatePrescriptionStatusDto dto)
        {
            var prescription =
                await _prescriptionRepository.GetByIdForUpdateAsync(id);
            ...
            {
                throw new InvalidOperationException(
                    "Đơn thuốc tư vấn trực tuyến không thuộc luồng cấp thuốc trực tiếp tại nhà thuốc.");
            }

            var currentStatus =
                prescription.Status?.Trim().ToLowerInvariant() ?? "pending";
            ...
            if (currentStatus == "pending")
            {
                ...
                {
                    throw new InvalidOperationException(
                        "Đơn thuốc đang chờ chỉ có thể được duyệt hoặc hủy.");
                }
            }
            ...
            else if (currentStatus == "approved")
            {
                ...
                {
                    throw new InvalidOperationException(
                        "Đơn thuốc đã duyệt chỉ có thể hủy hoặc thực hiện cấp thuốc.");
                }
            }
            ...
            {
                throw new InvalidOperationException(
                    "Đơn thuốc ở trạng thái hiện tại không thể thay đổi.");
            }
            ...
            if (updated == null)
                throw new Exception(
                    "Không thể lấy đơn thuốc sau khi cập nhật.");

            return MapToDto(updated);
        }

        public async Task<PrescriptionDto?> DispenseAsync(
            Guid id,
            Guid pharmacistUserId,
            DispensePrescriptionDto dto)
        {
            var prescription =
                await _prescriptionRepository.GetByIdForUpdateAsync(id);
            ...
            {
                throw new InvalidOperationException(
                    "Đơn thuốc tư vấn trực tuyến chỉ được sử dụng để bệnh nhân tự mua hoặc đặt hàng.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "Chỉ đơn thuốc đã được duyệt mới có thể cấp thuốc.");
            }

            // BR-DISP-002: Phải có danh sách thuốc.
            if (dto.Items == null || dto.Items.Count == 0)
                throw new ArgumentException(
                    "Vui lòng nhập danh sách thuốc được cấp.");

            if (dto.Items.Count != prescription.PrescriptionItems.Count)
                throw new InvalidOperationException(
                    "Phải xử lý đầy đủ tất cả thuốc trong đơn.");

            if (dto.Items.Any(item =>
                    item.PrescriptionItemId == Guid.Empty ||
                    item.ProductId == Guid.Empty))
            {
                throw new ArgumentException(
                    "Thông tin thuốc được cấp không hợp lệ.");
            }

            var prescriptionItemIds = dto.Items
                .Select(item => item.PrescriptionItemId)
                .ToList();

            // BR-DISP-003: Không gửi trùng PrescriptionItem.
            if (prescriptionItemIds.Distinct().Count() !=
                prescriptionItemIds.Count)
            {
                throw new InvalidOperationException(
                    "Không được xử lý cùng một thuốc trong đơn nhiều lần.");
            }

            var actualItemIds = prescription.PrescriptionItems
                .Select(item => item.Id)
                .ToHashSet();

            // BR-DISP-004: Item request phải thuộc Prescription hiện tại.
            if (prescriptionItemIds.Any(
                    itemId => !actualItemIds.Contains(itemId)))
            {
                throw new InvalidOperationException(
                    "Có thuốc không thuộc đơn thuốc đang xử lý.");
            }

            // BR-DISP-005: Không được cấp lại đơn đã có dữ liệu cấp.
            if (prescription.PrescriptionItems.Any(
                    item => item.PrescriptionDispenseItem != null))
            {
                throw new InvalidOperationException(
                    "Đơn thuốc này đã có dữ liệu cấp thuốc.");
            }
            ...
            var products =
                await _productRepository.GetByIdsForUpdateAsync(productIds);

            if (products.Count != productIds.Count)
                throw new ArgumentException(
                    "Có Product được chọn không tồn tại.");

            var dispenseItems =
                new List<PrescriptionDispenseItem>();
            ...
            foreach (var requestItem in dto.Items)
            {
                if (requestItem.QuantityDispensed <= 0)
                    throw new ArgumentException(
                        "Số lượng cấp phải lớn hơn 0.");

                var prescriptionItem =
                    prescription.PrescriptionItems.First(
                        item => item.Id == requestItem.PrescriptionItemId);
                ...
                if (!product.MedicationId.HasValue ||
                    product.MedicationId.Value != prescriptionItem.MedicationId)
                {
                    throw new InvalidOperationException(
                        $"Product '{product.Name}' không thuộc Medication '{prescriptionItem.Medication.Name}'.");
                }
                ...
                {
                    throw new InvalidOperationException(
                        $"Product '{product.Name}' không đủ tồn kho. Tồn hiện tại: {currentStock}.");
                }
            ...
            if (updated == null)
                throw new Exception(
                    "Không thể lấy đơn thuốc sau khi cấp thuốc.");
        ...
        private static PrescriptionCheckoutOptionDto MapToCheckoutOptionDto(
            Prescription prescription)
        {
            ...
            return new PrescriptionCheckoutOptionDto
            {
                Id = prescription.Id,
                ...
                Items = prescription.PrescriptionItems
                    .Select(item => new PrescriptionCheckoutItemDto
                    {
                        MedicationId = item.MedicationId
                    })
                    .ToList()
        ...
        private static PrescriptionDto MapToDto(Prescription prescription)
        {
            ...
            return new PrescriptionDto
            {
                ...
                AppointmentScheduledAt =
                    AsUtc(prescription.Appointment.ScheduledAt),
                AppointmentType = prescription.Appointment.Type,
        ...
        private static PrescriptionItemDto MapItemToDto(
            PrescriptionItem item)
        {
            return new PrescriptionItemDto
            {
                ...
                Dispense = MapDispenseToDto(
                    item.PrescriptionDispenseItem)
            };
        }

        private static PrescriptionDispenseItemDto? MapDispenseToDto(
            PrescriptionDispenseItem? dispense)
        {
            if (dispense == null)
                return null;
